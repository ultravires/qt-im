			...
			<el-form v-model="loginForm" class="login-form">
				...
				<el-form-item prop="password">
					<el-input
						...
						<i slot="prefix" class="el-input__icon" :class="isPasswordFocus ? 'el-icon-unlock' : 'el-icon-lock'"></i>
						<i slot="suffix" class="el-input__icon el-icon-view" @click="isShowPassword = !isShowPassword"></i>
					</el-input>
				</el-form-item>
				<el-form-item>
...
<script>
import Vue from 'vue';
import { Avatar, Form, FormItem, Input, Button, Checkbox, Message } from 'element-ui';
...
export default {
	...
	beforeCreate() {
		this.$qtBridge && this.$qtBridge.resize(IMPanel.LoginPanel.width, IMPanel.LoginPanel.height);
	},
	...
	created() {
		...
		this.isRememberUsername && (this.loginForm.username = AES.decrypt(cipherUsername, secret).toString(enc.Utf8));
		this.isRememberPassword && (this.loginForm.password = AES.decrypt(cipherPassword, secret).toString(enc.Utf8));
	},
	...
	methods: {
		...
		login() {
			...
			login({ username, password }).then(res => {
				...
				this.$qtBridge && this.$qtBridge.open({
					key: 'MAIN_WINDOW',
		...
		setPrefToLocal() {
			window.localStorage.setItem('isRememberUsername', JSON.stringify(this.isRememberUsername));
			window.localStorage.setItem('isRememberPassword', JSON.stringify(this.isRememberPassword));
		},

		getPrefFromLocal() {
			let isRememberUsername = JSON.parse(window.localStorage.getItem('isRememberUsername'));
			let isRememberPassword = JSON.parse(window.localStorage.getItem('isRememberPassword'));
...
.login-panel {
	...
	background-image: url('~@/assets/login/bg_login_background.jpg');
